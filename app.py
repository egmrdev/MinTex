...
st.set_page_config(page_title="MinTexto v5", page_icon="🧠", layout="wide", initial_sidebar_state="expanded")
...
html, body, div, p, span, h1, h2, h3, h4, h5, h6, button, input, label { 
    font-family: 'Inter', sans-serif; 
}

/* Proteger los iconos de Streamlit para que no se rompan */
...
.hero-sub {
    ...
    font-weight: 400; margin-bottom: 1.5rem; letter-spacing: 0.3px;
}
...
.mcard {
    ...
    border: 1px solid #252550; border-radius: 14px;
    padding: 1rem 1.2rem; text-align: center;
    box-shadow: 0 4px 24px rgba(129,140,248,0.06);
}
.mcard-label { font-size: 0.68rem; color: #6366a0; text-transform: uppercase; letter-spacing: 1.5px; }
.mcard-value {
...
def fig_perdida(hist):
    ...
    fig.update_layout(
        ...
        font=dict(color="#d0d0f0", family="Inter"),
...
def fig_barras(datos, titulo, es_pct=False):
    ...
    fig.update_layout(
        title=dict(text=titulo, font=dict(color="#a5b4fc", size=12, family="Inter")),
        xaxis=dict(color="#555580", gridcolor="#141432", zeroline=False,
                   tickformat=".2%" if es_pct else ".2f"),
        yaxis=dict(color="#c4c4e4", tickfont=dict(size=11, family="Inter")),
        plot_bgcolor="#080814", paper_bgcolor="#080814",
        font=dict(color="#d0d0f0", family="Inter"),
...
with st.sidebar:
    ...
    fuente = st.radio("", ["📂 Subir archivo", "✏️ Escribir texto"], label_visibility="collapsed")
    st.markdown("---")
...
if iniciar and texto_corpus.strip():
    st.markdown("---")
    st.markdown('<div class="sec-head">⚙️ Entrenando Nuevo Modelo...</div>', unsafe_allow_html=True)
    ...
    st.warning("⚠️ Ingresa o sube un corpus primero.")

# Si hay un modelo entrenado, mostrar todas las gráficas (ya sea recién entrenado o recuperado del state)
if st.session_state.modelo:
    ...
    with c1:
        consulta = st.text_input("Palabra:", placeholder="ej: mineria",
                                 label_visibility="collapsed")
    with c2:
    ...
    if buscar and consulta.strip():
        ...
        if pal not in m["vocab"]:
            ...
            with st.expander("Tabla de Similitud coseno"):
                ...
                df["Similitud"] = df["Similitud"].map("{:.4f}".format)
                st.dataframe(df, use_container_width=True, hide_index=True)

            with st.expander("Tabla de Prediccion de contexto"):
                df2 = pd.DataFrame(ctx_d, columns=["Prob","Palabra"])[["Palabra","Prob"]]
                df2["Probabilidad"] = df2["Prob"].map("{:.2%}".format)
                st.dataframe(df2[["Palabra","Probabilidad"]], use_container_width=True, hide_index=True)
